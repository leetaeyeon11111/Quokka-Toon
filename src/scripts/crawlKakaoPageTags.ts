/**
 * 카카오페이지 태그 크롤링 → 결과 CSV 저장 (DB 쓰기 없음, sqlite는 읽기만)
 *
 * 실행:
 *   npx ts-node -r tsconfig-paths/register src/scripts/crawlKakaoPageTags.ts
 *
 * 흐름:
 *   1) targets_kakaopage.csv 읽기 (MySQL 대상: webtoon_id, product_name)
 *   2) sqlite(kakao_page_webtoon)에서 product_name ↔ title 매칭 → id에서 seriesId 추출
 *   3) getContentAbout(seriesId) 호출 → theme_keyword_list 태그 수집
 *   4) (webtoon_id, tag) 형태로 결과 CSV에 append
 *   5) 매칭 실패 / 태그 없음은 별도 CSV로 기록
 *   6) 재실행 시 이미 처리한 webtoon_id는 건너뜀 (이어하기)
 *
 * 입력:  targets_kakaopage.csv  (webtoon_id \t product_name, 탭 구분, 헤더 1줄)
 * 출력:  kakaopage_tags_result.csv   (webtoon_id,tag)
 *        kakaopage_failed.csv        (webtoon_id,product_name,reason)
 */
...
import { getContentAbout } from '../modules/kakao-page/functions/kakaoPageApi'; // ← 실제 경로에 맞게 수정

// ───────────────────────────────────────────────────────────
// 경로 / 설정
// ───────────────────────────────────────────────────────────
const INPUT_CSV = path.resolve(process.cwd(), 'targets_kakaopage.csv');
const OUTPUT_CSV = path.resolve(process.cwd(), 'kakaopage_tags_result.csv');
const FAILED_CSV = path.resolve(process.cwd(), 'kakaopage_failed.csv');
...
// ───────────────────────────────────────────────────────────
// sqlite에서 카카오페이지 title → seriesId 맵 구축
//   id 형식: "kakopage_57925268" → seriesId 57925268
// ───────────────────────────────────────────────────────────
async function buildSeriesIdMap(): Promise<{
  exact: Map<string, number>;
  normed: Map<string, number>;
}> {
  const rows: { id: string; title: string }[] = await AppDataSource.query(
    `SELECT id, title FROM kakao_page_webtoon`,
  );
  ...
  for (const r of rows) {
    ...
    if (r.title != null) {
      if (!exact.has(r.title)) exact.set(r.title, seriesId);
      const n = norm(r.title);
      if (n && !normed.has(n)) normed.set(n, seriesId);
    }
  }
  return { exact, normed };
}
...
async function main() {
  ...
  console.info('✅ sqlite 연결됨 (seriesId 매칭용, 읽기 전용)');

  const { exact, normed } = await buildSeriesIdMap();
  console.info(
    `🗂️  sqlite 카카오페이지 매핑: 정확 ${exact.size}건 / 정규화 ${normed.size}건`,
  );
  ...
  for (const t of targets) {
    ...
    let seriesId = exact.get(t.product_name);
    if (seriesId === undefined) seriesId = normed.get(norm(t.product_name));

    if (seriesId === undefined) {
      fs.appendFileSync(
        FAILED_CSV,
        `${t.webtoon_id},${csvField(t.product_name)},no_seriesid_in_sqlite\n`,
        'utf-8',
      );
      failCount++;
      console.warn(`⏭️  ${prefix} — sqlite에 seriesId 없음`);
      continue;
    }

    try {
      const res = await getContentAbout(seriesId);
      const keywordList = res.data?.result?.theme_keyword_list || [];
      const tags = keywordList
        .map((k) => (k?.title || '').trim())
        .filter(Boolean);
      ...
      if (tags.length === 0) {
        ...
        failCount++;
        console.warn(`⏭️  ${prefix} — 태그 0개 (seriesId=${seriesId})`);
