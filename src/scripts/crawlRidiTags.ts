/**
 * 리디북스 태그 크롤링 → 결과 CSV 저장 (DB 연결 없음)
 *
 * 실행:
 *   npx ts-node -r tsconfig-paths/register src/scripts/crawlRidiTags.ts
 *
 * 입력:
 *   targets_ridi.csv  (webtoon_id \t title, 탭 구분, 헤더 1줄)
 *
 * 출력:
 *   ridi_tags_result.csv   (webtoon_id,tag  — 태그마다 한 줄)
 *   ridi_failed.csv        (webtoon_id,title,reason)
 *
 * 동작:
 *   1) 입력 CSV에서 대상(webtoon_id, title) 확보
 *   2) findRidiId(title) → bookId → fetchRidiDetail(bookId) → tags[]
...
import { findRidiId, fetchRidiDetail } from '../modules/ridi/ridiApi'; // ← 실제 경로에 맞게 수정

// ───────────────────────────────────────────────────────────
// 경로 / 설정
// ───────────────────────────────────────────────────────────
const INPUT_CSV = path.resolve(process.cwd(), 'targets_ridi.csv');
const OUTPUT_CSV = path.resolve(process.cwd(), 'ridi_tags_result.csv');
const FAILED_CSV = path.resolve(process.cwd(), 'ridi_failed.csv');

const DELAY_MS = 4000; // 요청 간 딜레이(ms)
...
async function main() {
  ...
  for (const t of targets) {
    ...
    try {
      // 1) 제목으로 리디 book_id 찾기
      const bookId = await findRidiId(t.title);
      if (!bookId) {
        fs.appendFileSync(
          FAILED_CSV,
          `${t.webtoon_id},${csvField(t.title)},id_not_found\n`,
          'utf-8',
        );
        failCount++;
        console.warn(`⏭️  ${prefix} — book_id 못 찾음`);
      ...
      const tags = (detail?.tags || []).map((s) => s.trim()).filter(Boolean);
      ...
      if (tags.length === 0) {
        ...
        failCount++;
        console.warn(`⏭️  ${prefix} — 태그 0개 (bookId=${bookId})`);
        await sleep(DELAY_MS);
        continue;
      }
