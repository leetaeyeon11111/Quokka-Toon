/**
 * 투믹스 태그 크롤링 → 결과 CSV 저장 (DB 연결 없음)
 *
 * 실행:
 *   npx ts-node -r tsconfig-paths/register src/scripts/crawlToomicsTags.ts
 *
 * 입력:
 *   targets_toomics.csv  (webtoon_id \t title, 탭 구분, 헤더 1줄)
 *
 * 출력:
 *   toomics_tags_result.csv   (webtoon_id,tag  — 태그마다 한 줄)
 *   toomics_failed.csv        (webtoon_id,title,reason)
 *
 * 동작:
 *   1) 입력 CSV에서 대상(webtoon_id, title) 확보
 *   2) getToomicsAbout(title) → tags[]
...
import { getToomicsAbout } from '../modules/toomics/toomicsApi'; // ← 실제 경로에 맞게 수정

// ───────────────────────────────────────────────────────────
// 경로 / 설정
// ───────────────────────────────────────────────────────────
const INPUT_CSV = path.resolve(process.cwd(), 'targets_toomics.csv');
const OUTPUT_CSV = path.resolve(process.cwd(), 'toomics_tags_result.csv');
const FAILED_CSV = path.resolve(process.cwd(), 'toomics_failed.csv');

const DELAY_MS = 2500; // 요청 간 딜레이(ms). 429 뜨면 늘리세요.
...
async function main() {
  ...
  for (const t of targets) {
    ...
    try {
      const about = await getToomicsAbout(t.title);
      const tags = (about?.tags || []).map((s) => s.trim()).filter(Boolean);

      if (tags.length === 0) {
        fs.appendFileSync(
          FAILED_CSV,
          `${t.webtoon_id},${csvField(t.title)},no_tags_or_id\n`,
          'utf-8',
        );
        failCount++;
