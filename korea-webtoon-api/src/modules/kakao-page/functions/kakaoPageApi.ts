...
axiosRetry(kakaoPageBffApi, {
  retries: 3,
  retryDelay: (retryCount) => retryCount * 3_000,
  onRetry: (retry, _, config) => {
    console.error(`🚧 [KAKAO_PAGE_BFF] ${config.url} - retry: ${retry}`);
  },
});

interface GetContentAboutResponse {
  result: {
    description: string | null;
    theme_keyword_list: { title: string }[];
  };
}

export const getContentAbout = (seriesId: number) => {
  console.info(`⌛️ [KAKAO_PAGE] seriesId: ${seriesId} - about(태그) 요청`);
  return kakaoPageBffApi.get<GetContentAboutResponse>(
    `/content/about?series_id=${seriesId}`,
  );
};
